...
export class AthleteController {
  ...
  //find all users
  @Get()
  async findAll() : Promise<Athlete[]|any> {
        return await this.athleteService.findAll();
    }
 
  // find user by uuid
  @Get(':athlete_id')
  async findOne(@Param('athlete_id',ParseIntPipe) athlete_id : number) : Promise<Athlete|any> {
      return await this.athleteService.findOne(athlete_id);
    }

  @Delete(':athlete_id')
  async deleteOne(@Param('athlete_id',ParseIntPipe) athlete_id: number): Promise<DeleteResult | any> {
